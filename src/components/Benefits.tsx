...
export const Benefits: React.FC<BenefitsProps> = ({ benefits = BENEFITS_DATA }) => {
  ...
  return (
    <section
      id="beneficios"
      className="py-16 lg:py-20 bg-white border-y border-[#e8ddca]"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center max-w-2xl mx-auto mb-12 lg:mb-16">
          <span className="text-xs font-montserrat font-bold uppercase tracking-widest text-[#6F9E73] block mb-2">
            Nuestros Pilares
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {list.map((benefit) => (
            <div
              ...
              className="bg-[#FAF6F0] rounded-xl p-6 sm:p-7 border border-[#e8ddca]/80 transition-all duration-300 hover:shadow-md hover:-translate-y-1 group flex flex-col items-center text-center"
            >
              {/* Icon Container */}
              <div className="w-16 h-16 rounded-full bg-[#6F9E73]/20 flex items-center justify-center mb-5 group-hover:bg-[#6F9E73]/30 transition-colors duration-200">
                {getIcon(benefit.iconName)}
              </div>

              {/* Title */}
              <h3 className="text-lg sm:text-xl font-bold font-montserrat text-[#285943] mb-2.5">
                {benefit.title}
              </h3>
